...
bool RuntimeController::DispatchStylusAction(
    const std::string& preferred_action_data) {
  platform_data_.stylus_action = preferred_action_data;

  if (auto* platform_configuration = GetPlatformConfigurationIfAvailable()) {
    platform_configuration->DispatchStylusAction(platform_data_.stylus_action);
    return true;
  }
